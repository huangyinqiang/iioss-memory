...
public class RedisCommonHashMemory extends RedisCommonMemory {
    ...
    public List<byte[]> getBytes(String ... keys) {
        try(StatefulConnection<String, byte[]> connection = super.connect()) {
            RedisHashCommands<String, byte[]> cmd = (RedisHashCommands)super.sync(connection);
            return cmd.hmget(this.region, CollectionUtil.newArrayList(keys).stream().toArray(String[]::new)).stream().map(kv -> kv.hasValue()?kv.getValue():null).collect(Collectors.toList());
        }
    }
