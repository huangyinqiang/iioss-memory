...
public class RedisCommonGenericMemory extends RedisCommonMemory {
    ...
    @Override
    public List<byte[]> getBytes(String ... keys) {
        try(StatefulConnection<String, byte[]> connection = super.connect()) {
            RedisStringCommands<String, byte[]> cmd = (RedisStringCommands)super.sync(connection);
            return cmd.mget(CollectionUtil.newArrayList(keys).stream().map(k -> _key(k)).toArray(String[]::new)).stream().map(kv -> kv.hasValue()?kv.getValue():null).collect(Collectors.toList());
        }
    }
    ...
    public void delete(String ...  keys) {
        try(StatefulConnection<String, byte[]> connection = super.connect()) {
            RedisKeyCommands<String, byte[]> cmd = (RedisKeyCommands)super.sync(connection);
            cmd.del(CollectionUtil.newArrayList(keys).stream().map(k -> _key(k)).toArray(String[]::new));
        }
    }
